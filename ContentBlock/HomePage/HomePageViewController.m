                          ...
                          if(tempJSON[@"type"]) {
                              ...
                              if([tempAdsModel.type isEqualToString:@"native"] && tempAdsModel.isOpen) {
                                  ...
                                  dispatch_async(dispatch_get_main_queue(), ^{
                                      ...
                                      self->adsurl = [NSURL URLWithString:tempAdsModel.url];
                                      self->imgAds.userInteractionEnabled = YES;
                                      [self->imgAds addGestureRecognizer:[[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(actImage:)]];
...
- (void)actImage:(id)sender {
    ...
    if(adsurl != nil) {
        if([[UIApplication sharedApplication] canOpenURL:adsurl]) {
            if(@available(iOS 10.0, *))
                [[UIApplication sharedApplication] openURL:adsurl options:@{} completionHandler:nil];
            else
                [[UIApplication sharedApplication] openURL:adsurl options:@{} completionHandler:nil];
        }
    }
